...
class AddonData
{
  public:
	...
	{
		napi_status status;
		status = napi_create_reference(env, cons, 1, &refCons);
	...
	napi_value GetCons(napi_env env)
	{
		napi_status status;
		napi_value nv;
		status = napi_get_reference_value(env, refCons, &nv);
		assert(napi_ok == status);
		return nv;
	}
	...
	{
		napi_status status;
		status = napi_create_reference(env, exp, 0, &refExports);
		assert(napi_ok == status);
	}
	...
	napi_value GetExports(napi_env env)
	{
		napi_status status;
		napi_value nv;
		status = napi_get_reference_value(env, refExports, &nv);
		assert(napi_ok == status);
		return nv;
	}
	...
	{
		napi_status status;
		status = napi_create_reference(env, ad, 1, &refAd);
		assert(napi_ok == status);
	}
	...
	napi_value GetAddon(napi_env env)
	{
		napi_status status;
		napi_value nv;
		status = napi_get_reference_value(env, refAd, &nv);
		assert(napi_ok == status);
		return nv;
	}
...
napi_value New(napi_env env, napi_callback_info info)
{
	...
	if (is_instance)
	{
		...
		napi_status status = napi_wrap(env, thisArg, (void *)point, PtFinalizer, (void *)"New myPoint", NULL);
		assert(napi_ok == status);
...
// typedef duration<double, std::ratio<1, 1000>> doubleMs;
...
void execAsync(napi_env env, void *data)
{
	...
	// time_t tmStart = time_point_cast<duration<time_t>>(start).time_since_epoch().count();

	clock_type::duration hidt = start.time_since_epoch();
	std::chrono::system_clock::time_point sc(duration_cast<seconds>(hidt));
	time_t tmStart = std::chrono::system_clock::to_time_t(sc);
	std::cout << "\n异步工作开始于：" << ctime(&tmStart);
	std::chrono::system_clock::time_point sctp = std::chrono::system_clock::now();
	time_t sctm = std::chrono::system_clock::to_time_t(sctp);
	std::cout << "\n用 system_clock 显示时间: " << ctime(&sctm);
...
{
	WORK_DATA *wd = (WORK_DATA *)data;
	if (napi_cancelled == status)
	{
	}
	else
	{
	...
	// time_point<high_resolution_clock, doubleMs> msEnd = time_point_cast<doubleMs>(end);
	duration<double, std::ratio<1, 1000>> wdt = end - wd->start;

	// time_t tmEnd = time_point_cast<duration<time_t>>(end).time_since_epoch().count();

	time_t tmEnd = duration_cast<seconds>(end.time_since_epoch()).count();

	std::cout << "\n异步工作结束于：" << ctime(&tmEnd);
	std::cout << "\nC-Addon的异步工作共执行 " << wdt.count() << " ms\n";

	napi_delete_async_work(env, wd->work);
	delete wd;
}

napi_value TestAsync(napi_env env, napi_callback_info info)
{
	napi_status status;
	napi_value argv[2];
	size_t argc = sizeof(argv) / sizeof(argv[0]);
	napi_value asyncResource = nullptr;
	napi_value asyncResName = nullptr;
	...
	asyncResName = argv[1];
	char name[32];
	napi_get_value_string_utf8(env, asyncResName, name, sizeof(name) / sizeof(name[0]), nullptr);

	std::cout << name << std::endl;
	asyncResource = argv[0];

	WORK_DATA *data = new WORK_DATA;

	napi_create_async_work(env, asyncResource, asyncResName, execAsync, complAsync,
						   (void *)data, &(data->work));
...
napi_value init(napi_env env, napi_value exports)
{
	...
	napi_property_descriptor descr1[] = {// {"utf8name", name, method, getter, setter, value, attributes, data}
										 ...
										 {"testAsync", NULL, TestAsync, NULL, NULL, NULL, napi_default, NULL},
